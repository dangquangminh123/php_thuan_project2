<?php 
    ...
    if(isset($_POST['register']))
    {
        ...
        //kiểm trả user có tồn tại hay chưa
        ...
        if(mysqli_num_rows($u_exist)==0){
            ...
            if($u_fetch['is_verified']==0){
                ...
                if(!send_mail($data['email'],$token,'account_recovery')){
                    ...
                    $date = date("Y-m-d");
                    $query = mysqli_query($con,"UPDATE `user_cred` SET `token`='$token',`t_expre`='$date' 
                        WHERE `id`='$u_fetch[id]'");
                    if($query){
                        echo 1;
                    }
    ...
    if(isset($_POST['recover_user']))
    {
        ...
        $enc_pass = password_hash($data['pass'],PASSWORD_BCRYPT);
        $query = "UPDATE `user_cred` SET `password`=? `token`=?, `t_expre`=? 
